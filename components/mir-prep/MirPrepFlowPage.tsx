...
import EnarmPrepForm, { FormAnswers } from '../enarm-prep/EnarmPrepForm'; // Reusing ENARM form
...
type MirPrepPhase = 'FORM' | 'TEST_INTRO' | 'TEST_QUESTIONS' | 'TEST_RESULTS' | 'PLAN_GENERATION' | 'PLAN_DISPLAY' | 'PLAN_ERROR';

const MirPrepFlowPage: React.FC = () => {
  ...
  const examType: ExamType = 'MIR'; // Specific to this flow

  const handleFormComplete = (answers: FormAnswers) => {
    setFormAnswers(answers);
    setCurrentPhase('TEST_INTRO'); 
    console.log("MIR Prep Form Answers:", answers);
  };
  ...
  return (
    <div className="fixed inset-0 bg-bg-light dark:bg-bg-dark text-text-primary-light dark:text-text-primary-dark flex flex-col items-center justify-center p-4 overflow-y-auto custom-scrollbar">
      <button
        ...
        aria-label="Salir de la preparación MIR y volver al dashboard"
      >
      ...
      {currentPhase === 'TEST_INTRO' && (
        <div className="text-center animate-fade-in-up max-w-xl w-full">
          <h1 className="text-3xl sm:text-4xl font-bold text-primary mb-4">🎓 ¡Perfecto!</h1>
          <p className="text-lg sm:text-xl text-text-secondary-light dark:text-text-secondary-dark mb-8">
            Has completado la información inicial para tu preparación MIR. Ahora, generaremos tu plan de estudio personalizado.
          ...
          >
            Generar Mi Plan de Estudio MIR
          </button>
        </div>
      )}
      ...
      {currentPhase === 'TEST_QUESTIONS' && (
        <div className="text-center animate-fade-in-up">
          <h1 className="text-3xl font-bold text-primary mb-6">Fase de Test Diagnóstico (MIR)</h1>
           <p className="text-text-secondary-light dark:text-text-secondary-dark">
            (Esta sección está en desarrollo)
          </p>
           <button 
            onClick={() => setCurrentPhase('PLAN_GENERATION')} 
       ...
       {currentPhase === 'PLAN_GENERATION' && (
        <div className="text-center animate-fade-in-up flex flex-col items-center">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-primary mb-4">Generando tu Plan de Estudio Personalizado para el MIR...</h1>
          <p className="text-text-secondary-light dark:text-text-secondary-dark mb-6 max-w-md">
            Estamos analizando tus respuestas para crear la mejor estrategia de preparación para el MIR. Esto podría tomar un momento.
          </p>
      ...
      {currentPhase === 'PLAN_DISPLAY' && generatedPlan && (
        <div className="text-center animate-fade-in-up max-w-2xl w-full">
          <h1 className="text-3xl sm:text-4xl font-bold text-primary mb-4">🎉 ¡Tu Plan de Estudio MIR está Listo!</h1>
          <div className="bg-surface-light dark:bg-surface-dark p-6 rounded-xl shadow-lg border border-border-light dark:border-border-dark mb-8">
            ...
            <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark mb-2">
              <strong>Fecha Objetivo:</strong> {generatedPlan.targetDate ? new Date(generatedPlan.targetDate).toLocaleDateString('es-ES', { year: 'numeric', month: 'long', day: 'numeric' }) : 'No especificada'}
            </p>
      ...
      {currentPhase === 'PLAN_ERROR' && (
        <div className="text-center animate-fade-in-up max-w-xl w-full">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-red-500 mb-4">Error al Generar el Plan MIR</h1>
          <p className="text-md text-text-secondary-light dark:text-text-secondary-dark mb-6">
            Lo sentimos, no pudimos generar tu plan de estudio para el MIR en este momento.
          </p>
...
export default MirPrepFlowPage;
