...
import UsmlePrepFormXp from './UsmlePrepFormXp';
...
import { StudyPlan, ExamType, UsmleXpFormAnswers, AnyExamFormAnswers } from '../../types';
import LoadingSpinner from '../LoadingSpinner';

type UsmlePrepPhase = 'FORM' | 'PLAN_GENERATION' | 'PLAN_ERROR';

const UsmlePrepFlowPageXp: React.FC = () => {
  const [currentPhase, setCurrentPhase] = useState<UsmlePrepPhase>('FORM');
  const [formAnswers, setFormAnswers] = useState<Partial<UsmleXpFormAnswers>>({});
  ...
  // examType will be determined by formAnswers.section1.usmleStepToPrep

  useEffect(() => {
    const locationState = location.state as { planIdToEdit?: string; initialData?: UsmleXpFormAnswers };
    if (locationState?.planIdToEdit) {
      const planToEdit = getStudyPlanById(locationState.planIdToEdit);
      if (planToEdit && (planToEdit.examType === 'USMLE Step 1' || planToEdit.examType === 'USMLE Step 2 CK' || planToEdit.examType === 'USMLE Step 3')) {
        setFormAnswers(planToEdit.userInput as UsmleXpFormAnswers);
  ...
  const handleFormComplete = (answers: UsmleXpFormAnswers) => {
    setFormAnswers(answers);
    setCurrentPhase('PLAN_GENERATION'); 
    console.log("USMLE XP Form Answers (Specific):", answers);
  };
  ...
  useEffect(() => {
    if (currentPhase === 'PLAN_GENERATION' && formAnswers.section1?.usmleStepToPrep) {
      const currentExamType = formAnswers.section1.usmleStepToPrep;
      generateStudyPlan(formAnswers as AnyExamFormAnswers, currentExamType)
        ...
        .catch(error => {
          console.error(`Error generating USMLE XP study plan for ${currentExamType}:`, error);
  ...
  const getPageTitle = () => {
    let baseTitle = "Plan USMLE Personalizado";
    if (formAnswers.section1?.usmleStepToPrep) {
      baseTitle = `Plan ${formAnswers.section1.usmleStepToPrep} Personalizado`;
    }
      ...
      {currentPhase === 'FORM' && (
        <div className="w-full h-full flex flex-col items-center justify-center pt-16 sm:pt-20">
            <UsmlePrepFormXp 
                ...
                initialData={formAnswers as UsmleXpFormAnswers} 
            />
        </div>
      )}
      ...
      {currentPhase === 'PLAN_GENERATION' && (
        <div className="text-center animate-fade-in-up flex flex-col items-center justify-center h-full">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-primary mb-4 font-poppins">
            Generando tu {formAnswers.section1?.usmleStepToPrep || "USMLE"} Plan Personalizado...
          </h1>
          <p className="text-text-secondary-dark mb-8 max-w-md">
            Blasto AI está diseñando tu estrategia para el USMLE. Esto podría tomar un momento.
          </p>
      ...
      {currentPhase === 'PLAN_ERROR' && (
        <div className="text-center animate-fade-in-up max-w-xl w-full flex flex-col items-center justify-center h-full pt-16 sm:pt-20">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-red-500 mb-4 font-poppins">
            Error al Generar el Plan {formAnswers.section1?.usmleStepToPrep || "USMLE"}
          </h1>
...
export default UsmlePrepFlowPageXp;